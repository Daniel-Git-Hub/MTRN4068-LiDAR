...
)
{
    ...
    if (handle==NULL || feat==NULL || (n<1)) return ;
...
)
{
    ...
    if (handle==NULL || pos==NULL || (n<1)) return ;
...
)
{
    ...
    if (handle==NULL || point==NULL || (n<1)) return ;
...
)
{
  ...
  if (label_x!=NULL) {
    ...
    char temp[] = "X";
      gnuplot_set_xlabel(handle, temp);
  }
  if (label_y!=NULL) {
      gnuplot_set_ylabel(handle, label_y);
  } else {
    char temp[] = "Y";
    gnuplot_set_ylabel(handle, temp);
  }
  if (y==NULL) {
...
char const * gnuplot_tmpfile(gnuplot_ctrl * handle)
{
    static char const * tmp_filename_template = "gnu/gnuplot_tmpdatafile_XXXXXX";
